...
export default function RepairChecklist({ steps }: Props) {
  const [states, setStates] = useState<Record<number, StepState>>(
    () => Object.fromEntries(steps.map(s => [s.id, { checked: false, note: '', flagged: false, photos: [], timeMinutes: 0, expandedNote: false }]))
  );
  ...
  const handlePhoto = (stepId: number) => {
    ...
    input.onchange = (e: any) => {
      ...
      if (file) {
        update(stepId, { photos: [...(states[stepId]?.photos || []), file.name] });
      }
    };
    input.click();
  };
  ...
  return (
    <Card className="bg-card border-border">
      ...
      <CardContent className="space-y-1 pb-4">
        {steps.map((step, idx) => {
          const s = states[step.id] || { checked: false, note: '', flagged: false, photos: [], timeMinutes: 0, expandedNote: false };
          return (
